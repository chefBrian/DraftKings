...
namespace DraftKings
{
    public static class Football
    {
        ...
        {
            Console.WriteLine();
            var pointUrl = "https://www.draftkings.com/lineup/getavailableplayers?draftGroupId=" + groupId;
            ...
            foreach (var p in playerList)
            {
                ...
                request.UserAgent = "Chrome 41.0.2228.0";
            ...
            validPlayers.AddRange(playerList.Where(e => e.pn == "RB").OrderByDescending(e => e.ep).Take(9));
            validPlayers.AddRange(playerList.Where(e => e.pn == "WR").OrderByDescending(e => e.ep).Take(9));
            validPlayers.AddRange(playerList.Where(e => e.pn == "TE").OrderByDescending(e => e.ep).Take(5));
            validPlayers.AddRange(playerList.Where(e => e.pn == "DST").OrderByDescending(e => e.ep).Take(5));
            
            Console.WriteLine();
            Console.WriteLine("Combinations Running...");
            var possibleTeams = new Combinations<Player>(validPlayers, 9, GenerateOption.WithoutRepetition).ToList();
            //var possibleTeams = MathFun.Combs(validPlayers.OrderByDescending(p => p.pn).ToList(), 9).ToList();
            ...
            Console.WriteLine(possibleTeams.Count + " possible teams.");
